<div class="mb-4">
          ...
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            ...
            <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3 text-sm">
                  @if ($producto->imagen != "")
                   <img width="80" src="{{asset('resources/productos/'.$producto->imagen)}}">
                   @endif
                </td>
                ...
                <td class="px-4 py-3">
                  ...
                  <td class="px-6 py-3 text-sm">
                      <div class="flex items-center space-x-4 text-sm">
                          <button type="button" wire:click="MostrarProductos({{$producto->id}})" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"  aria-label="Edit" >
  ...
  <x-jet-dialog-modal wire:model="mod">
    <x-slot name="title"> @if($title != "") {{ $title}} @else Agregar Producto @endif</x-slot>
    <x-slot name="content">
    ...
    @if($title != "Actualizar Producto")
    <x-jet-danger-button class="ml-2" wire:click="Save()" wire:loading.attr="disabled">
        Crear Producto
    </x-jet-danger-button>
    @else
    <x-jet-danger-button class="ml-2" wire:click="Update({{$Productosid}})" wire:loading.attr="disabled">
        Actualizar Producto
    </x-jet-danger-button>
    @endif
    </x-slot>
