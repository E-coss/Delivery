<div class="mb-4">
    ...
    <div class="grid w-full overflow-hidden rounded-lg shadow-xs">
          ...
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            ...
            <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3 text-sm">
                  @if ($Slide->imagen != "")
                   <img width="80" src="{{asset('resources/Slides/'.$Slide->imagen)}}">
                   @endif
                </td>
                  
                  <td class="px-4 py-3 text-sm">
                    @if($Slide->estado == "Activo")
                  ...
                  <td class="px-6 py-3 text-sm">
                      <div class="flex items-center space-x-4 text-sm">
                          <button type="button" wire:click="MostrarSlides({{$Slide->id}})" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"  aria-label="Edit" >
      ...
      {{ $Slides->links('components.pagination-tailwind') }}

      
  </div>

  <x-jet-dialog-modal wire:model="mod">
    <x-slot name="title"> @if($title != "") {{ $title}} @else Agregar Slide @endif</x-slot>
    <x-slot name="content">
        ...
        <div class="flex flex-wrap -mx-3 mb-3">
                ...
                <div class="w-full px-3 mb-6 md:mb-0">
                  ...
                  <div class="space-y-1 text-center">
                    ...
                    @if ($imagen == "" && $temporal != "")
                    Imagen del Slide
                   <img class="block" src="{{asset('resources/slides/'.$temporal)}}">
                   @endif 
    ...
    @else
    <x-jet-danger-button class="ml-2" wire:click="Update({{$Slideid}})" wire:loading.attr="disabled">
        Actualizar Slide
    </x-jet-danger-button>
    @endif
    </x-slot>
